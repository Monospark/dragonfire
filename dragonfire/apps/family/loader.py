...
#                     "library":   library,
        ...
        for (key, value) in dragonfire.__dict__.items():
            if isinstance(value, type) and issubclass(value, ActionBase):
                namespace[key] = value
        namespace["Repeat"] = dragonfire.Repeat
    ...
    def build(self, call_info):
        ...
        if call_info.arguments:
            raise SyntaxError("Invalid arguments for dictation extra: %r"
                              % (call_info.arguments,))
        print "just build", dragonfire.Dictation(name)
        return dragonfire.Dictation(name)
    ...
    def build(self, call_info):
        ...
        if call_info.arguments:
            raise SyntaxError("Invalid arguments for integer extra: %r"
                              % (call_info.arguments,))
        print "just build", dragonfire.Integer(name=name, min=min, max=max)
        return dragonfire.Integer(name=name, min=min, max=max)
